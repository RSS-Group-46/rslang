...
import {
  selectStatistic,
  selectLearnedWords,
} from '../../../redux/selectors/statistic.selectors';
import './Graph.scss';
...
const procentWordsOfAnyText = (x) => {
  if (x > 70) {
    return Math.round(wordsOfAnyTextLog(x));
  }
  if (x <= 70) {
    return Math.round(wordsOfAnyTextLine(x));
  }
  return 0;
};
...
const Graph = () => {
  ...
  function drawGraph() {
    ...
    for (let i = 0; i <= 100; i += 20) {
      context.fillText(
        i + ' %',
        3,
        heightGraph - 3 - (i / 100) * heightGraph * 0.9 - 20,
      );
      context.moveTo(0, heightGraph - (i / 100) * heightGraph * 0.9 - 20);
      context.lineTo(
        widthGraph,
        heightGraph - (i / 100) * heightGraph * 0.9 - 20,
      );
    }
    ...
    for (i = 0; i <= 5000; i += 1) {
      if (i > 70) {
        context.lineTo(
          35 + (i / 5000) * widthGraph * 0.9,
          heightGraph - 20 - (wordsOfAnyTextLog(i) / 100) * heightGraph * 0.9,
        );
      }
      if (i <= 70) {
        context.lineTo(
          35 + (i / 5000) * widthGraph * 0.9,
          heightGraph - 20 - (wordsOfAnyTextLine(i) / 100) * heightGraph * 0.9,
        );
      }
    }
    ...
    for (j = 0; j <= learnedWordsNumber; j += 1) {
      if (j > 70) {
        context.lineTo(
          35 + (j / 5000) * widthGraph * 0.9,
          heightGraph - 20 - (wordsOfAnyTextLog(j) / 100) * heightGraph * 0.9,
        );
      }
      if (i <= 70) {
        context.lineTo(
          35 + (j / 5000) * widthGraph * 0.9,
          heightGraph - 20 - (wordsOfAnyTextLine(j) / 100) * heightGraph * 0.9,
        );
      }
    }
  ...
  useEffect(() => {
    drawGraph();
  }, [statistic, learnedWordsNumber, drawGraph]);

  return (
    <>
      <p className="statistic__text">
        Всего изучено слов {learnedWordsNumber}. Теперь Вы должны понимать{' '}
        {procentWordsOfAnyText(learnedWordsNumber)} % слов любого текста.
      </p>
